"""Rule-based backtester that computes equity curves and performance metrics."""
import pandas as pd
import numpy as np
...
def metrics(equity):
    ...
    if ret.empty: return {'CAGR':None,'Sharpe':None,'MaxDD':None}
    ann = 252
    cagr = (equity.iloc[-1]/equity.iloc[0])**(ann/len(equity))-1
    sharpe = (ret.mean()/ret.std(ddof=0))*np.sqrt(ann)
    dd = (equity/equity.cummax()-1).min()
    return {'CAGR':round(cagr*100,2),'Sharpe':round(sharpe,2),'MaxDD':round(dd*100,2)}

def run(df):
    eq_curves=[]; stats=[]
    for t in df['Ticker'].unique():
        d = df[df['Ticker']==t].reset_index(drop=True)
        eq = 1.0
        curve=[eq]
        for i in range(1,len(d)):
            ...
            eq *= (1+r)
            curve.append(eq)
        eq_s = pd.Series(curve, index=d['Date'], name=t)
        eq_curves.append(eq_s)
        m = metrics(eq_s)
        stats.append({'Ticker':t,**m})
    pd.concat(eq_curves,axis=1).to_csv(EQT)
    pd.DataFrame(stats).to_csv(OUT,index=False)
    print(f'âœ… Backtest results saved to {OUT} and equity to {EQT}')
