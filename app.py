...
st.markdown("<style>body{background-color:#0E1117;color:white;} .stPlotlyChart{background-color:#0E1117;}</style>", unsafe_allow_html=True)
...
def load_df(path, parse_dates=None):
    ...
    try: return pd.read_csv(path, parse_dates=parse_dates)
...
if sent.empty:
    st.warning("⚠️ Run the pipeline first to generate sentiment data."); st.stop()
...
# ---------- Tabs ----------
t1, t2, t3 = st.tabs(["📈 Sentiment Trend", "💵 Sentiment vs Return", "📊 Correlation Summary"])
...
with t1:
    ...
    if df.empty:
        ...
        fig = px.line(df, x="Date", y="SentimentScore", markers=True,
                      title=f"{ticker} Sentiment Trend",
                      color_discrete_sequence=["#00CC96"])
        fig.update_layout(template="plotly_dark", yaxis_title="Sentiment (-1 → +1)",
                          xaxis_title=None, height=400, showlegend=False)
...
with t2:
    ...
    if dfm.empty:
        ...
        fig2 = px.scatter(dfm, x="SentimentScore", y="Return",
        ...
        st.dataframe(dfm[["Date","SentimentScore","Return"]].tail(10), use_container_width=True)
...
with t3:
    ...
    if corr.empty:
        ...
        fig3 = px.bar(corr_sorted, x="Ticker", y="Correlation",
                      color="Correlation",
...
# --- extra data loaders ---
def safe_load(p, parse_dates=None):
    return pd.read_csv(p, parse_dates=parse_dates) if os.path.exists(p) else pd.DataFrame()

# --- create advanced tabs ---
t4, t5, t6 = st.tabs(["🧭 Indicators", "🤖 Predictions (WF-CV)", "📊 Backtest & Signals"])
...
with t4:
    ...
    if tech.empty:
        ...
        fig = px.line(df_t, x="Date", y=["RSI14","MACD","BB_width"],
...
with t6:
    ...
    if not eq.empty:
        for col in [c for c in eq.columns if c.lower() not in ["unnamed: 0"]]:
            fig = px.line(eq, x=eq.columns[0], y=col, title=f"Equity Curve — {col}")
